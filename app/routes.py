...
def index():
    form = SearchStation()
    followed = Metar.query.join(Follow, (Follow.code == Metar.station_id)).distinct(Metar.station_id).all()
...
def register():
    ...
    if form.validate_on_submit():
        user = User(username=form.username.data, email=form.email.data, phone=form.phone.data, first_name=form.first_name.data, last_name=form.last_name.data)
